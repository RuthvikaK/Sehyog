...
def load_gemma_model(model_name="unsloth/gemma-7b-it-bnb-4bit"):
...
def run_gemma(tokenizer, model, question, choices, max_tokens=80):
...
if __name__ == "__main__":
    try:
        if len(sys.argv) < 3:
            raise ValueError("❌ Usage: python gemma_infer_single_query.py '<question>' '[\"choice1\", \"choice2\"]'")

        question = sys.argv[1]
        choices = eval(sys.argv[2])  # e.g. "[\"Glue the legs\", \"Add a shim\"]"

        print("🚀 Question received:", question)
        print("🧩 Choices:", choices)

        tokenizer, model = load_gemma_model()
        _ = run_gemma(tokenizer, model, "Warmup", ["Option A", "Option B"], max_tokens=10)

        response = run_gemma(tokenizer, model, question, choices)
